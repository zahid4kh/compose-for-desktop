...
export default function MarkdownPreview({ markdown }: MarkdownPreviewProps) {
  ...
  useEffect(() => {
    ...
    const convertMarkdownToHtml = (md: string) => {
      ...
      processed = processed.replace(
        /<li(.*?)>([\s\S]*?)(?=<li|$)/g,
        '<ul class="list-disc mb-4"><li$1>$2</li></ul>'
      );
      processed = processed.replace(/<\/ul>\n<ul[^>]*>/g, "");

      processed = processed.replace(
        /\[\!\[([^\]]+)\]\(([^)]+)\)\]\(([^)]+)\)/g,
        '<a href="$3" class="inline-block mr-2"><img src="$2" alt="$1" class="h-5" /></a>'
      );
  ...
  if (!mounted) {
    return (
      <div className="bg-[#1e1e1e] rounded-md p-4 h-full overflow-auto text-white">
        Loading...
      </div>
    );
  }
